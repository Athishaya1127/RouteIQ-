    ...
    <div className="w-80 bg-white shadow-xl flex flex-col h-full z-10 border-r border-slate-200/80">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div>
          ...
          {placementMode === 'customer' && (
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 mt-3">
              <label className="block text-xs font-medium text-slate-700 mb-1">Assign Customer to Shop</label>
              <select 
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm space-y-3">
              ...
              <div className="text-sm text-slate-600 mb-2">
                <p><strong>Delivery Partner:</strong> {result.partner_id}</p>
                <p><strong>Assigned Shop:</strong> {result.shop_id}</p>
                <p><strong>Customers:</strong> {result.customers.join(', ')}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-slate-50 p-2 rounded">
                  ...
                  <p className="font-medium text-slate-900">{result.total_duration} min</p>
                </div>
                <div className="bg-slate-50 p-2 rounded">
                  <p className="text-xs text-slate-500 uppercase tracking-wider">Fuel Cost</p>
                  <p className="font-medium text-slate-900">₹{result.fuel}</p>
                </div>
                <div className="bg-slate-50 p-2 rounded">
                  <p className="text-xs text-slate-500 uppercase tracking-wider">Total Cost</p>
                  <p className="font-medium text-slate-900">{result.cost}</p>
                </div>
              </div>
              ...
              <div className="pt-2 border-t">
                ...
                <div className="flex flex-col gap-2">
                  {result.sequence.map((locationId, i) => {
                        ...
                        <div>
                          ...
                          <div className="flex justify-between items-center text-xs">
                            <div className="flex gap-2 text-slate-500 font-medium">
                              ...
                              <span>•</span>
                              <span>{segment.duration.toFixed(1)} min</span>
                            </div>
                            ...
                            <span className={`px-1.5 py-0.5 text-[9px] font-extrabold rounded uppercase tracking-wider
                              ...
                              {isHigh ? `Severe Traffic (${(segment.congestion * 100).toFixed(0)}%)` : 
                               isMod ? `Moderate (${(segment.congestion * 100).toFixed(0)}%)` : 
                               `Clear (${(segment.congestion * 100).toFixed(0)}%)`}
                            </span>
                          </div>
                        </div>
                      </div>
